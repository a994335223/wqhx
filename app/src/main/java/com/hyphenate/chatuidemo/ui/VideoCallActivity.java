...
//                callHelper.startVideoRecord(PathUtil.getInstance().getVideoPath().getAbsolutePath());
    ...
    void startMonitor() {
        ...
        new Thread(() -> {
            while (monitor) {
                runOnUiThread(() -> {
                    monitorTextView.setText("宽x高："
                            + callHelper.getVideoWidth() + "x" + callHelper.getVideoHeight()
                            + "\n延迟：" + callHelper.getVideoLatency()
                            + "\n帧数：" + callHelper.getVideoFrameRate()
                            + "\n丢失的：" + callHelper.getVideoLostRate()
                            + "\n本地比特率：" + callHelper.getLocalBitrate()
                            + "\n远程比特率：" + callHelper.getRemoteBitrate()
                            + "\n" + recordString);
